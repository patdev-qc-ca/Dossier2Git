// Dossier2Git 1.0.4.cpp : Ce fichier contient la fonction 'main'. L'exécution du programme commence et se termine à cet endroit.
// © Patrice Waechter-Ebling 2024 mise a jour 2025
//
#include <windows.h>
#include <winhttp.h>
#include <iostream>
...
void GetVariablesCPU(char** env) {
	char titre[128] = { 0 };
	for (char** e = env; *e != nullptr; ++e) {
		if (split(*e, '=')[0] == "PROCESSOR_IDENTIFIER") { strcpy(IDENTIFIER, split(*e, '=')[1].c_str()); }
		if (split(*e, '=')[0] == "PROCESSOR_ARCHITECTURE") { strcpy(ARCHITECTURE, split(*e, '=')[1].c_str()); }
		if (split(*e, '=')[0] == "PROCESSOR_ARCHITEW6432") { strcpy(ARCHITEW6432, split(*e, '=')[1].c_str()); }
		if (split(*e, '=')[0] == "PROCESSOR_LEVEL") { strcpy(LEVEL, split(*e, '=')[1].c_str()); }
		if (split(*e, '=')[0] == "PROCESSOR_REVISION") { strcpy(REVISION, split(*e, '=')[1].c_str()); }
	}
	wsprintfA(titre, " %s  %s  %s  %s%s", IDENTIFIER, ARCHITECTURE, ARCHITEW6432, LEVEL, REVISION);
...
{
	std::cout << " Dossier2Git\tv:1.04\t(C) Patrice Waechter-Ebling 2024-2025" << std::endl;
	std::string Jeton(ClefJeton);
	int i = 0;
	GetVariablesCPU(env);
	for (char** e = env; *e != nullptr; ++e) {
		i++;
		if (split(*e, '=')[0] == "OS") {std::cout <<split(*e, '=')[1].c_str()<< "\n";}
		if (Jeton.c_str() == *e) {
			std::cout << "Chiffrage SSH: " << Jeton.substr(13, 32) << std::endl;
		}

	}
	printf(" %s  %s  %s  %s%s\n%s\t%s", IDENTIFIER, ARCHITECTURE, ARCHITEW6432, LEVEL, REVISION,env[52],env[53]);
	...
	if (pidl != nullptr) {
		...
		\"description\": \"Dépot créé par Dossier2Git pour %s\",\
		\"private\": false,\
		\"auto_init\": true})", NomDepot, NomDepot);
			HINTERNET hConnexion = WinHttpOpen(L"Dossier2Git", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
			if (!hConnexion) {
				std::cerr << "Echec lors de l'execution de l'api WinHttpOpen\n";
				return 1;
